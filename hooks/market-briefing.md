...
| **미드데이 브리핑** | 12:30 KST | 장중 시장 점검 | 당일 데이터 |
| **애프터 마켓 브리핑** | 18:00 KST | 장 마감 후 시장 요약 | 당일 데이터 |
...
### 미드데이 브리핑 (장중)
```bash
python scripts/main.py --type midday
```
...
python scripts/main.py --type midday --ai
...
1. 사용자의 요청이 모닝/미드데이/애프터마켓 중 어느 타입인지 판단
   - 명시적으로 지정한 경우 해당 타입 사용
   - 지정하지 않은 경우: 현재 시간이 10:00 KST 이전이면 morning, 10:00~15:00이면 midday, 이후이면 aftermarket
...
매일 **08:00 모닝** / **12:30 미드데이** / **18:00 애프터마켓** (KST) 자동 생성
